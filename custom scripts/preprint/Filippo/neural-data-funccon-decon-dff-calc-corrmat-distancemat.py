...
from preprint.Filippo.utilities import remove_unwanted_mice


def filter_always_present(dic):
    ...
    filtered = {k: v.dropna() for k, v in dic.items()}
    return filtered
...
if __name__ == '__main__':
    ...
    savestr = r'C:\Users\hheise.UZH\Desktop\preprint\Filippo\correlation_matrices'
    if not os.path.isdir(savestr):
        os.mkdir(savestr)

    # plt.style.use('code/08012024/plotstyle.mplstyle')

    dff_path = r'C:\Users\hheise.UZH\PycharmProjects\Caiman\custom scripts\preprint\Filippo\neural_data/dff_all_cells_normal.pkl'
    decon_path = r'C:\Users\hheise.UZH\PycharmProjects\Caiman\custom scripts\preprint\Filippo\neural_data/decon_all_cells_normal.pkl'
    sa_path = r'C:\Users\hheise.UZH\PycharmProjects\Caiman\custom scripts\preprint\Filippo\neural_data/spatial_activity_maps_dff_all_cells.pkl'
    ...
    """
    with open(f'{savestr}/cluster-correlation-dff.pkl', 'rb') as handle:
        testload_dff = pickle.load(handle)

    with open(f'{savestr}/cluster-correlation-decon.pkl', 'rb') as handle:
        testload_decon = pickle.load(handle)
    """
