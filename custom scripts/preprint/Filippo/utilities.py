...
def df_corr_result(res,
                   key='corr'):  # since the default key of df_corr_result is 'corr', what is written are the unclustered correlation matrices!
...
def get_poststroke_mapping():
    ...
    #    rel_days [(rel_days == 2) | (rel_days == 3)] == 3
    ...
    # rel_days[(rel_days == 15) | (rel_days == 15) | (rel_days == 16)] = 15
...
def plot_qty_correlation_vectors(df, savename, savestr=None, callable=np.mean, labelstr='{}', xlabel='Relative days',
                                 ylabel='Correlation',
    ...
    if normalized:
        for i in toplot_qtys.index:
            ...
            toplot_qtys.loc[i] *= 1 / toplot_qtys.loc[
                i, l3_poststroke_sessions].mean()  # normalize to last 3/4 prestroke sessions
...
def plot_qty_correlation_mean_std(df, savestr=None, savename=None, groups=None, group_colors='k', callable=np.mean,
                                  labelstr='{}', xlabel='Relative days', ylabel='Correlation',
                                  titlestr=None, normalized=False,
                                  legend=False,
                                  align=False):
    if savestr is None:
    ...
    for groupcount, raw_group in enumerate(division):
        ...
        clean_mean_idx, clean_mean = align_row(means[groupcount]) if align else (
        means[groupcount].index, means[groupcount].values)
        clean_std_idx, clean_std = align_row(stds[groupcount]) if align else (
        stds[groupcount].index, stds[groupcount].values)
...
def ax_plot_coor_fit(axes, mousecount, mouse, row, xframe, yframe, xrange=np.array([-0.2, 0.8])):
...
def get_group_results_slope_corr(group, pre_early_late_fit_corrcoef):
    ...
    res_slope = np.array(
        [(pre_early_late_fit_corrcoef[m]['fit_pre_early'].slope, pre_early_late_fit_corrcoef[m]['fit_pre_late'].slope,
          pre_early_late_fit_corrcoef[m]['fit_early_late'].slope) for m in group])

    res_corr = []
    for m in group:
        try:
            res_corr.append((pre_early_late_fit_corrcoef[m]['corr_pre_early'].statistic,
                             pre_early_late_fit_corrcoef[m]['corr_pre_late'].statistic,
                             pre_early_late_fit_corrcoef[m]['corr_early_late'].statistic))
        except:
    ...
    for m in group:
        try:
            res_pval.append((pre_early_late_fit_corrcoef[m]['corr_pre_early'].pvalue,
                             pre_early_late_fit_corrcoef[m]['corr_pre_late'].pvalue,
                             pre_early_late_fit_corrcoef[m]['corr_early_late'].pvalue))
        except:
...
def plot_quantiles_with_data(ax, categories, data_arrays, titlestr='', ylabel='', xlabel=''):
    ...
    max_range = max(
        [np.max(np.array(data)[~np.isnan(data)]) - np.min(np.array(data)[~np.isnan(data)]) for data in data_arrays])
...
def melt_join_dataframes(df_list, names_list, suffix=''):
    dfs_melted = [df.reset_index().rename(columns={'index': 'mouse'}).melt(id_vars=['mouse'], var_name='day',
                                                                           value_name=f'{name}_{suffix}') for df, name
                  in zip(df_list, names_list)]
