...
def get_group_results_slope_corr(group, pre_early_late_fit_corrcoef):
    ...
    res_slope = np.array([(pre_early_late_fit_corrcoef[m]['fit_pre_early'].slope, pre_early_late_fit_corrcoef[m]['fit_pre_late'].slope,
                           pre_early_late_fit_corrcoef[m]['fit_early_late'].slope) for m in group])
    
    res_corr = []
    for m in group:
        try:
            res_corr.append((pre_early_late_fit_corrcoef[m]['corr_pre_early'].statistic, pre_early_late_fit_corrcoef[m]['corr_pre_late'].statistic,
    ...
    for m in group:
        try:
            res_pval.append((pre_early_late_fit_corrcoef[m]['corr_pre_early'].pvalue, pre_early_late_fit_corrcoef[m]['corr_pre_late'].pvalue,
